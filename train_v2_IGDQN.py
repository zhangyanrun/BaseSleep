...
def main(run_dir):
    ...
    # 【消融核心 2】：为每个 Mesh 分配一个独立的 DRL 智能体 (不共享参数和经验池)
    print("正在初始化 I-GDQN (无参数共享) 独立多智能体系统...")
    agents_dict = {}
    for mesh_id in mesh_ids:
        agents_dict[mesh_id] = PS_DQNAgent(
    ...
    for epoch in range(TRAIN_PARAMS['num_epochs']):
        ...
        for mesh_id in mesh_ids:
            # 唤醒当前 Mesh 的专属智能体
            current_agent = agents_dict[mesh_id]
            
            features, adj = env.reset(mesh_id)
            done = False
            while not done:
                actions = current_agent.select_actions(features, adj)
                next_features, next_adj, reward, done, info = env.step(actions)
                
                # 经验只存入自己的专属记忆池，并用自己的网络学习
                current_agent.store_transition(features, adj, actions, reward, next_features, next_adj, done)
                current_agent.learn()
            ...
            if (epoch + 1) % TRAIN_PARAMS['target_update'] == 0:
                current_agent.update_target()
                
        print(f"Epoch {epoch+1} | 平均 Reward: {np.mean(epoch_reward):.4f}")

    # 保存所有独立的模型
    for mesh_id, agent in agents_dict.items():
        torch.save(agent.policy_net.state_dict(), os.path.join(models_dir, f'igdqn_mesh_{mesh_id}.pth'))

if __name__ == "__main__":
    timestamp = time.strftime("%Y%m%d_%H%M%S") + "_V2_IGDQN"
